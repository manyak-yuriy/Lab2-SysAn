...
tVector get_c_vec(int Y_num, const tVector B[], const tVector XX[], const tVector &lambda, const tVector &a, int p1, int p2, int p3, int dim1, int dim2, int dim3, int q0, int type)
{
	...
	for (int j = 0; j < 3; j++)
	{
		A(q, j) = PHI(j, q, XX, lambda, a, p1, p2, p3, dim1, dim2, dim3, type);
	}
	...
	switch (Vec_num)
	{
		case 0:
		{
				  for (int k = 0; k < dim1; k++)
					  res += a.elem(k) * PSI(0, k, q, XX, lambda, p1, p2, p3, dim1, dim2, dim3, type);
				  break;
		}
		case 1:
		{
				  for (int k = 0; k < dim2; k++)
					  res += a.elem(k + dim1) * PSI(1, k, q, XX, lambda, p1, p2, p3, dim1, dim2, dim3, type);
				  break;
		}
		case 2:
		{
				  for (int k = 0; k < dim3; k++)
					  res += a.elem(k + dim1) * PSI(2, k, q, XX, lambda, p1, p2, p3, dim1, dim2, dim3, type);
				  break;
		}
	}
	return res;
...
tVector get_a_vec(int Y_num, const tVector B[], const tVector XX[], const tVector &lambda, int p1, int p2, int p3, int dim1, int dim2, int dim3, int q0, int type)
{
	...
	for (int q = 0; q < q0; q++)
	{
		for (int k = 0; k < dim1; k++)
		{
			A(q, k) = PSI(0, k, q, XX, lambda, p1, p2, p3, dim1, dim2, dim3, type);
		}
		for (int k = 0; k < dim2; k++)
		{
			A(q, dim1 + k) = PSI(1, k, q, XX, lambda, p1, p2, p3, dim1, dim2, dim3, type);
		}
		for (int k = 0; k < dim3; k++)
		{
			A(q, dim1 + dim2 + k) = PSI(2, k, q, XX, lambda, p1, p2, p3, dim1, dim2, dim3, type);
		}
	}
...
tVector get_lambda_break_down(int Y_num, const tVector B[], const tVector XX[], int type, int p1, int p2, int p3, int dim1, int dim2, int dim3, int q0)
{
	...
	for (int i = 0; i < q0; i++)
	   for (int X_var = 0; X_var < dim1; X_var++)
	      for (int k = 0; k < p1; k++)
		     A1(i, k + p1 * (X_var)) = Polynom(type, k, XX[i].elem(X_var));
   ...
   for (int k = 0; k < p2; k++)
	   A2(i, k + p2 * (X_var)) = Polynom(type, k, XX[i].elem(X_var));
   ...
   for (int k = 0; k < p3; k++)
	   A3(i, k + p3 * (X_var)) = Polynom(type, k, XX[i].elem(X_var));
...
tVector get_lambda(int Y_num, const tVector B[], const tVector XX[], int type, int p1, int p2, int p3, int dim1, int dim2, int dim3, int q0)
{
	...
	for (int i = 0; i < q0; i++)
	{
		...
		for (int X_var = 0; X_var < dim1; X_var++)
		for (int k = 0; k < p1; k++)
			A(i, k + p1 * (X_var)) = Polynom( type, k, XX[i].elem(X_var) );
		// second vector
		for (int X_var = 0; X_var < dim2; X_var++)
		for (int k = 0; k < p2; k++)
			A(i, k + dim1 * p1 + p2 * (X_var)) = Polynom( type, k, XX[i].elem(dim1 + X_var) );
		// third vector
		for (int X_var = 0; X_var < dim3; X_var++)
		for (int k = 0; k < p3; k++)
			A(i, k + dim1 * p1 + dim2 * p2 + p3 * (X_var)) = Polynom(type, k, XX[i].elem(dim1 + dim2 + X_var));
	}
	...
	case 0:
	{
		...
		for (int k = 0; k < p1; k++)
			res += lambda.elem(k + p1 * (Var_num)) 
				* Polynom(type, k, XX[q].elem(Var_num));
		break;
	}
	case 1:
	{
	   // second vector
	    for (int k = 0; k < p2; k++)
			res += lambda.elem(k + dim1 * p1 + p2 * (Var_num))
				* Polynom(type, k, XX[q].elem(dim1 + Var_num));
		break;
	}
	case 2:
	{
		...
		for (int k = 0; k < p3; k++)
			res += lambda.elem(k + dim1 * p1 + dim2 * p2 + p3 * (Var_num)) 
				* Polynom(type, k, XX[q].elem(dim1 + dim2 + Var_num));
		break;
	}
	...
	int q, tVector XX[])
{
	...
	for (int vec_num = 0; vec_num < dim1; vec_num++)
	{
		res += (c.elem(0) * a.elem(vec_num) * PSI(0, vec_num, q, XX, lambda, p1, p2, p3, dim1, dim2, dim3, type));
	}

	// second vector
	for (int vec_num = 0; vec_num < dim2; vec_num++)
	{
		res += (c.elem(1) * a.elem(vec_num + dim1) * PSI(1, vec_num, q, XX, lambda, p1, p2, p3, dim1, dim2, dim3, type));
	}
	// third vector
	for (int vec_num = 0; vec_num < dim3; vec_num++)
	{
		res += (c.elem(2) * a.elem(vec_num + dim1 + dim2) * PSI(2, vec_num, q, XX, lambda, p1, p2, p3, dim1, dim2, dim3, type));
	}
	return res;
}
